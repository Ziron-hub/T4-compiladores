...
class LASemantico(LAVisitor):
            ...
            if escopoAtual.contem(nome):
                ...
                if len(params) != len(ctx.expressao()):
                    ...
                    for i in range(len(params)):
                        if params[i].tipo != Analisador.verificar_tipo_expressao(
                            escopos=self.escopos, ctx=ctx.expressao()[i]
                        ):
                            erro = True
                if erro:
                    Analisador.adicionar_erro_semantico(
    ...
    # Visita nó declaracao de variavel
    def visitDeclaracao_variavel(self, contexto):
        ...
        for identificador in contexto.variavel().identificador():
            ...
            if escopoAtual.contem(nomeId):
                ...
                if tipo is not None:
                    escopoAtual.adicionar_tabela_simbolos(
                        nomeId, tipo, TabelaDeSimbolos.Estrutura.VARIAVEL
                    )
                else:
                    identTipo = (
                        contexto.variavel()
                    ...
                    if identTipo is not None:
                        ...
                        if escopoAtual.contem(nomeId):
                            ...
                            escopoAtual.adicionar_tabela_simbolos(
                                nomeId,
                                TabelaDeSimbolos.TipoLA.REGISTRADOR,
                                TabelaDeSimbolos.Estrutura.VARIAVEL,
                            )
                            if registros is not None:
                                for registro in registros:
                                    escopoAtual.adicionar_tabela_simbolos(
                                        nomeId + "." + registro.nome,
                                        registro.tipo,
                                        TabelaDeSimbolos.Estrutura.VARIAVEL,
                                    )
                        ...
                        for variavel in (
                            contexto.variavel().tipo().registro().variavel()
                        ):
                            tipoRegistros = Analisador.get_tipo(
                                variavel.tipo().getText()
                            )
                            for ident in variavel.identificador():
                                entrada = TabelaDeSimbolos.EntradaTabelaDeSimbolos(
                                    ident.getText(),
                                    tipoRegistros,
                                    TabelaDeSimbolos.Estrutura.VARIAVEL,
                                )
    ...
    # Visita nó de declaracao de tipo
    def visitDeclaracao_tipo(self, contexto):
        ...
        if escopoAtual.contem(identificador):
            ...
            if tipo is not None:
                escopoAtual.adicionar_tabela_simbolos(
                    identificador, tipo, TabelaDeSimbolos.Estrutura.TIPO
                )
    ...
    # Visita nó de declaracao global
    def visitDeclaracao_global(self, contexto):
        ...
        if escopoAtual.contem(contexto.IDENT().getText()):
            ...
            if contexto.getText().startswith("funcao"):
                tipoRetornoFuncao = Analisador.get_tipo(
                    contexto.tipo_estendido().getText()
                )
            ...
            escopoAtual.adicionar_tabela_simbolos(
                contexto.IDENT().getText(), tipoRetornoFuncao, estrutura
            )
                    ...
                    for id in param.identificador():
                        ...
                        if escopoAtual.contem(nomeId):
                            ...
                            if tipo is not None:
                                ...
                                escopoAtual.adicionar_entrada_tabela_simbolos(
                                    entradaTabela
                                )
                                escopoAntigo.adicionar_tipo_nome(
                                    contexto.IDENT().getText(), entradaTabela
                                )
                            else:
                                identTipo = (
                                    param.tipo_estendido().tipo_basico_ident().IDENT()
                                    if (
                                        param.tipo_estendido().tipo_basico_ident()
                                        is not None
                                        and param.tipo_estendido()
                                        .tipo_basico_ident()
                                        .IDENT()
                                        is not None
                                    )
            ...
            for escopo in self.escopos.obter_pilha():
                if escopo.contem(nomeVar):
                    tipoVar = Analisador.verificar_tipo_nome_var(
                        escopos=self.escopos, nomeVar=nomeVar
                    )
                    varNumeric = (
    ...
    # Visita nó de tipo basico
    def visitTipo_basico_ident(self, contexto):
        if contexto.IDENT() is not None:
            nome_tipo = contexto.IDENT().getText()
            contem = any(
                escopo.contem(nome_tipo) for escopo in self.escopos.obter_pilha()
            )
            if not contem:
                Analisador.adicionar_erro_semantico(
                    contexto.start, f"tipo {nome_tipo} nao declarado"
                )
